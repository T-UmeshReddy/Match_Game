...
class App extends Component {
  state = {
    ...
    score: 0,
    seconds: 60,
    currentTargetImage: imagesList[0],
    isGameOver: false,
  }
    ...
    this.timerId = setInterval(() => {
      this.setState(prevState => {
        if (prevState.seconds === 1) {
          clearInterval(this.timerId)
          return {seconds: 0, isGameOver: true}
        }
  ...
  onClickThumbnail = id => {
    const {currentTargetImage} = this.state
    if (id === currentTargetImage.id) {
      const randomIndex = Math.floor(Math.random() * imagesList.length)
      this.setState(prevState => ({
        score: prevState.score + 1,
        currentTargetImage: imagesList[randomIndex],
      }))
    } else {
      clearInterval(this.timerId)
  ...
  renderGameView = () => {
    const {activeTabId, score, seconds, currentTargetImage} = this.state
    ...
    return (
      <div className="bottom-container">
        <img
          ...
          alt="match"
          src={currentTargetImage.imageUrl}
        />
          ...
          {tabsList.map(eachItem => (
            <li className="tab-item" key={eachItem.tabId}>
              <button
                type="button"
                className={`button ${
                  activeTabId === eachItem.tabId ? 'active' : ''
                }`}
                onClick={() => this.onChangeTab(eachItem.tabId)}
              >
                <p className="tab-name">{eachItem.displayText}</p>
              </button>
            </li>
          ))}
        </ul>
        ...
        <ul className="thumbnail-container">
          {filteredList.map(eachItem => (
            <li className="thumbnail" key={eachItem.id}>
              <button
                type="button"
                className="button"
                onClick={() => this.onClickThumbnail(eachItem.id)}
              >
                <img
